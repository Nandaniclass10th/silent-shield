...
<body>
    <div class="container">
        ...
        <div class="project-title">
            ...
            <div style="font-size: 18px; color: #666; margin-top: 5px;">Offline Women Safety System</div>
        </div>
        ...
        <button class="emergency-button" id="emergencyBtn" onclick="sendEmergency()">
        ...
        <div class="contacts-section">
            ...
            <input type="tel" class="contact-input" id="contact1" placeholder="📱 Contact 1 (Mother): +91 98765 43210">
            <input type="tel" class="contact-input" id="contact2" placeholder="📱 Contact 2 (Father): +91 98765 43211">
            <input type="tel" class="contact-input" id="contact3" placeholder="📱 Contact 3 (Friend): +91 98765 43212">
        ...
        <div style="background: #f5f5f5; padding: 15px; border-radius: 10px; margin: 20px 0; font-size: 14px;">
            ...
            HTML5 • CSS3 • JavaScript • Geolocation API • SMS URI
        </div>
    ...
    <div class="alert-box" id="alertBox"><i class="fas fa-check-circle"></i> Test Alert</div>

    <script>
        let emergencyContacts = ['', '', ''];
        
        window.onload = function() {
            console.log("Silent Shield App Loaded");
            try {
                const savedContacts = localStorage.getItem('silentShieldContacts');
                if (savedContacts) {
                    emergencyContacts = JSON.parse(savedContacts);
                    document.getElementById('contact1').value = emergencyContacts[0] || '';
                    document.getElementById('contact2').value = emergencyContacts[1] || '';
                    document.getElementById('contact3').value = emergencyContacts[2] || '';
                }
            } catch (error) {
                console.log("No contacts saved yet");
            }
            setTimeout(() => {
                showAlert("Welcome to Silent Shield! Add contacts and test.");
            }, 1000);
        };
        
        function saveContacts() {
            emergencyContacts[0] = document.getElementById('contact1').value.trim();
            emergencyContacts[1] = document.getElementById('contact2').value.trim();
            emergencyContacts[2] = document.getElementById('contact3').value.trim();
            ...
            if (!hasValidContact) {
                showAlert("⚠️ Please add at least one valid phone number!");
                return;
            }
            
            localStorage.setItem('silentShieldContacts', JSON.stringify(emergencyContacts));
            showAlert("✅ Contacts saved successfully!");
        }
        
        function sendEmergency() {
            console.log("Emergency button pressed!");
            showAlert("🛑 TEST: Button is working! In real emergency, help would be sent.");
            
            const btn = document.getElementById('emergencyBtn');
            const originalHTML = btn.innerHTML;
            const originalBg = btn.style.background;
            
            btn.innerHTML = '<div class="emergency-icon">✅</div>TESTED!';
